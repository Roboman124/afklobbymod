...
public class HudRenderer {
    ...
    public static void render(MatrixStack matrices) {
        ...
        int x = 10, y = 10;
        client.textRenderer.drawWithShadow(matrices, Text.literal("AFK Leaderboard").styled(s -> s.withBold(true)), x, y, 0xFFD700);
        y += 12;
        for (int i = 0; i < names.length; i++) {
            String line = (i + 1) + ". " + names[i] + " " + format(times[i]);
            client.textRenderer.drawWithShadow(matrices, Text.literal(line), x, y, 0xFFFFFF);
            y += 10;
        }
    }
