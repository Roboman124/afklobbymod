            ...
            .then(CommandManager.literal("hud")
                .then(CommandManager.argument("enabled", BoolArgumentType.bool())
                    .executes(ctx -> {
                        ...
                        Networking.setHudEnabled(p, enabled);
                        ServerPlayNetworking.send(p, new Networking.HudSyncPayload(enabled, java.util.List.of()));
                        reply(ctx.getSource(), "HUD " + (enabled ? "enabled" : "disabled") + ".");
                        return 1;
                    })))
        );
    }
